...
def _parse_utc_datetime_str(v):
    ...
    if not isinstance(v, datetime):
        ...
        dt = datetime.strptime(
            dt, "%Y-%m-%dT%H:%M:%S.%f%z" if "." in dt else "%Y-%m-%dT%H:%M:%S%z"
        )
...
def _coerce_float_type(v):
    fv = float(v)
    if math.isnan(fv):
        return _PFX_FLOAT, b"NaN"
    if math.isinf(fv):
        return _PFX_FLOAT, b"Infinity" if fv > 0 else b"-Infinity"
...
def _coerce_double_type(v):
    fv = float(v)
    if math.isnan(fv):
        return _PFX_DOUBLE, b"NaN"
    if math.isinf(fv):
        return _PFX_DOUBLE, b"Infinity" if fv > 0 else b"-Infinity"
