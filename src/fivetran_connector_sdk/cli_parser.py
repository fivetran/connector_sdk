...
def _add_yes(subparser):
    subparser.add_argument(
        "--yes", action="store_true", help="Automatically answers yes to all confirmation prompts"
    )
...
def _add_hd_agent_id(subparser):
    subparser.add_argument(
        ...
        type=str,
        metavar="<hybrid-deployment-agent-id>",
    ...
    if subparser.get_default("_reject_proxy_flags"):
        subparser.add_argument(
            ...
            help=(
                argparse.SUPPRESS
                if hidden
                else "Proxy Agent ID for routing connector traffic through a Proxy Agent"
            ),
        )
        return
    ...
    if subparser.get_default("_reject_proxy_flags"):
        subparser.add_argument(
            ...
            help=(
                argparse.SUPPRESS
                if hidden
                else "Configuration key that contains the host details to proxy"
            ),
        )
        return
...
def _add_template(subparser):
    subparser.add_argument(
        "--template",
        type=str,
        default=TEMPLATE_CONNECTOR_PATH,
        metavar="<template>",
...
def _add_non_interactive(subparser):
    subparser.add_argument(
        "--non-interactive",
        action="store_true",
        help="Automatically accepts the default choice for all confirmation prompts",
    )
...
def create_argument_parser():
    ...
    subparsers.add_parser(
        "version", help=COMMAND_DESCRIPTIONS["version"], usage="fivetran version"
    )

    init_subparser = subparsers.add_parser(
        "init", help=COMMAND_DESCRIPTIONS["init"], usage="fivetran init [project_path] [options]"
    )
    ...
    debug_subparser = subparsers.add_parser(
        "debug",
        help=COMMAND_DESCRIPTIONS["debug"],
    ...
    deploy_subparser = subparsers.add_parser(
        "deploy",
        help=COMMAND_DESCRIPTIONS["deploy"],
    ...
    _add_configuration(
        deploy_subparser,
        "Path to configuration JSON file. On redeploy, existing configuration is preserved if none is provided.",
    )
    ...
    package_subparser = subparsers.add_parser(
        "package",
        help=COMMAND_DESCRIPTIONS["package"],
    ...
    reset_subparser = subparsers.add_parser(
        "reset",
        help=COMMAND_DESCRIPTIONS["reset"],
    ...
    subparsers.add_parser("help", help=COMMAND_DESCRIPTIONS["help"], usage="fivetran help")
    return parser
...
def intercept_unknown_command():
    ...
    for argv_index, token in enumerate(sys.argv[1:], start=1):
        ...
        if not suggest_correct_command(token):
            print_library_log(
                f"invalid command: {token}, see `fivetran help`", Logging.Level.SEVERE
            )
        sys.exit(1)
