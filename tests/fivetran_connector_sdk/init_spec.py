...
class TestInit(unittest.TestCase):
    ...
    @patch("fivetran_connector_sdk.helpers.get_input_from_cli", side_effect=["api_key", "group", "connection"])
    def test_main_deploy(self, mock_input, mock_state, mock_config, mock_find, mock_get_configuration):
        mock_connector = MagicMock()
        mock_find.return_value = mock_connector
        test_args = [
            "fivetran", "deploy", "--api-key", "api_key", "--destination", "group", "--connection", "connection",
    ...
    def test_main_errors_on_yes_and_non_interactive_together(self):
        ...
        test_args = ["fivetran", "deploy", "--api-key", "key", "--destination", "dest",
                     "--connection", "conn", "--yes", "--non-interactive"]
        with patch.object(sys, "argv", test_args), \
    ...
    def test_main_logs_deprecation_warning_for_force_flags(self):
        ...
        for force_flag in ("--force", "-f"):
            with self.subTest(force_flag=force_flag):
                mock_connector = MagicMock()
                test_args = [
                    "fivetran", "deploy", "--api-key", "api_key", "--destination", "group",
                    "--connection", "connection", force_flag
                ]
                with patch.object(sys, "argv", test_args), \
                        patch("fivetran_connector_sdk.__init__.find_connector_object", return_value=mock_connector), \
                        patch("fivetran_connector_sdk.__init__.get_destination_group", return_value="group"), \
                        patch("fivetran_connector_sdk.__init__.get_connection_name", return_value="connection"), \
    ...
    def test_main_does_not_log_deprecation_warning_for_non_interactive(self):
        ...
        test_args = [
            "fivetran", "deploy", "--api-key", "api_key", "--destination", "group",
            "--connection", "connection", "--non-interactive"
        ]
        with patch.object(sys, "argv", test_args), \
                patch("fivetran_connector_sdk.__init__.find_connector_object", return_value=mock_connector), \
                patch("fivetran_connector_sdk.__init__.get_destination_group", return_value="group"), \
        ...
        deprecated_warning_calls = [
            log_call for log_call in mock_log.call_args_list
    ...
    def test_schema_success(self, mock_process_tables, mock_print_log):
        ...
        with patch("fivetran_connector_sdk.TABLES", {"t1": "table1", "t2": "table2"}):
            with patch("fivetran_connector_sdk.common_pb2.TableList") as mock_table_list:
                with patch("fivetran_connector_sdk.connector_sdk_pb2.SchemaResponse") as mock_schema_response:
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    def test_schema_debug_stores_crash_report_without_changing_error_behavior(self, mock_print_log):
        connector = Connector(update=MagicMock(), schema=MagicMock(side_effect=ValueError("schema fail")))
        connector.configuration = {"foo": "bar"}
        ...
        try:
            with patch("fivetran_connector_sdk.build_debug_crash_report", return_value="SCHEMA REPORT") as mock_report:
                with self.assertRaises(RuntimeError) as cm:
                    connector.Schema(mock_request, mock_context)
        finally:
            constants.DEBUGGING = original_debugging
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    def test_schema_debug_preserves_original_error_when_crash_report_generation_fails(self, mock_print_log):
        connector = Connector(update=MagicMock(), schema=MagicMock(side_effect=ValueError("schema fail")))
        connector.configuration = {"foo": "bar"}
        connector.project_path = "/project"
        mock_request = MagicMock()
        ...
        constants.DEBUGGING = True
        try:
            with patch("fivetran_connector_sdk.build_debug_crash_report", side_effect=RuntimeError("report fail")):
                with self.assertRaises(RuntimeError) as cm:
                    connector.Schema(mock_request, mock_context)
        finally:
            constants.DEBUGGING = original_debugging
        ...
        self.assertTrue(any(
            "failed to build crash report" in str(call.args[0]) and call.args[1] == Logging.Level.WARNING
    ...
    def test_update_typeerror_none_iterable(self, mock_log):
        ...
        connector.update_method = MagicMock(side_effect=TypeError("'NoneType' object is not iterable"))
        mock_request = MagicMock()
    ...
    def test_update_debug_stores_crash_report_without_changing_error_behavior(self, mock_log):
        ...
        try:
            ...
            with patch("fivetran_connector_sdk.build_debug_crash_report", return_value="REPORT") as mock_report, \
                    patch("builtins.print") as mock_print, \
                    patch("fivetran_connector_sdk.get_memory_tracker", return_value=mock_memory_tracker):
                gen = connector.Update(mock_request, mock_context)
                with self.assertRaises(RuntimeError) as cm:
                    next(gen)
        finally:
        ...
        self.assertEqual(mock_report.call_args.kwargs["configuration"], {"api_key": "secret-token"})
        self.assertEqual(connector._debug_crash_report, "REPORT")
        mock_print.assert_not_called()

    @patch("fivetran_connector_sdk.print_library_log")
    def test_update_debug_preserves_original_error_when_crash_report_generation_fails(self, mock_log):
        connector = Connector(update=MagicMock(side_effect=ValueError("fail")))
        connector.project_path = "/project"
        mock_request = MagicMock()
        mock_request.configuration = {"api_key": "secret-token"}
    ...
    def test_run_debugging_false(self):
        connector = Connector(update=MagicMock())
        with patch("fivetran_connector_sdk.check_dict", side_effect=lambda x, *a, **k: x), \
                ...
                patch("fivetran_connector_sdk.connector_sdk_pb2_grpc.add_SourceConnectorServicer_to_server") as mock_add, \
            ...
            connector.run(port=12345, configuration={"foo": "bar"}, state={"baz": 1}, log_level=Logging.Level.FINE)
    ...
    def test_run_debugging_true(self):
        ...
        with patch("fivetran_connector_sdk.check_dict", side_effect=lambda x, *a, **k: x), \
            ...
            result = connector.run(port=54321, configuration={"a": 1}, state={"b": 2}, log_level=Logging.Level.INFO)
    ...
    def test_connector_configuration_form(self):
        ...
        mock_request = MagicMock()
        mock_context = MagicMock()
        with patch("fivetran_connector_sdk.protos.common_pb2.ConfigurationFormResponse") as mock_form_response:
            connector.ConfigurationForm(mock_request, mock_context)
            self.assertTrue(mock_form_response.called)

        with patch("fivetran_connector_sdk.Connector.debug") as mock_debug, \
                patch("fivetran_connector_sdk.print_library_log") as mock_log:
    ...
    def test_deploy_command_success(self):
        with patch("fivetran_connector_sdk.Connector.deploy") as mock_deploy, \
                patch("fivetran_connector_sdk.print_library_log") as mock_log:
            from fivetran_connector_sdk import Connector
            mock_deploy.return_value = None
            Connector.deploy(mock_deploy.__class__, "dummy_path", "dummy_key", "dummy_group", "dummy_conn", "dummy_hd_agent_id")
    ...
    def test_validate_requirements_file_called_when_interactive(self, mock_version_check, mock_get_group_info, mock_validate, mock_exists):
        from fivetran_connector_sdk import Connector
        ...
        with self.assertRaises(SystemExit) as cm:
            connector.deploy(project_path, "dummy_key", "dummy_group", "dummy_conn", "dummy_hd_agent_id", configuration={}, prompt_mode=PromptMode.INTERACTIVE)
    ...
    @patch("fivetran_connector_sdk.get_group_info", side_effect=SystemExit(1))
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_validate_pyproject_file_called_for_pyproject_toml(self, mock_version_check, mock_get_group_info, mock_validate_req, mock_validate_toml, mock_exists):
        from fivetran_connector_sdk import Connector
    ...
    @patch("fivetran_connector_sdk.get_group_info", side_effect=SystemExit(1))
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_with_non_interactive_runs_pyproject_validation(self, mock_version_check, mock_get_group_info, mock_validate_req,
                                                                    mock_validate_toml, mock_log, mock_exists):
        """Deploy with non_interactive and pyproject.toml present: validation runs with DEFAULT_ANSWER mode."""
        from fivetran_connector_sdk import Connector

        connector = Connector(update=MagicMock())
        with self.assertRaises(SystemExit):
            connector.deploy("/dummy/path", "dummy_key", "dummy_group", "dummy_conn", "dummy_hd_agent_id", configuration={}, prompt_mode=PromptMode.DEFAULT_ANSWER)
    ...
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_exits_when_configuration_is_none_for_new_connection(self, mock_version, mock_group, mock_connection, mock_validate, mock_exists):
        from fivetran_connector_sdk import Connector

        connector = Connector(update=MagicMock())
        with self.assertRaises(SystemExit) as cm:
            connector.deploy("/dummy/path", "dummy_key", "dummy_group", "dummy_conn", "dummy_hd_agent_id", configuration=None)
        self.assertEqual(cm.exception.code, 1)
    ...
    @patch("fivetran_connector_sdk.get_group_info", side_effect=SystemExit(1))
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_with_yes_runs_pyproject_validation(self, mock_version_check, mock_get_group_info, mock_validate_req,
                                                        mock_validate_toml, mock_log, mock_exists):
        ...
        connector = Connector(update=MagicMock())
        with self.assertRaises(SystemExit):
            connector.deploy("/dummy/path", "dummy_key", "dummy_group", "dummy_conn", "dummy_hd_agent_id",
    ...
    @patch("fivetran_connector_sdk.check_newer_version")
    @patch("fivetran_connector_sdk.ensure_tester_installed", return_value=("/tmp/tester/java", "/tmp/tester"))
    @patch("fivetran_connector_sdk.Connector.run")
    @patch("fivetran_connector_sdk.update_base_url_if_required")
    def test_debug_no_install_needed(self, mock_update_base_url, mock_run, mock_ensure, mock_check, mock_tester,
                                     mock_port, mock_exit, mock_validate, mock_log):
        mock_server = MagicMock()
    ...
    def test_debug_registers_crash_report_exit_hook_before_memory_reporting(self):
        ...
        with patch("fivetran_connector_sdk.print_library_log"), \
                patch("fivetran_connector_sdk.validate_requirements_file"), \
                patch("fivetran_connector_sdk.exit_check"), \
                patch("fivetran_connector_sdk.get_available_port", return_value=55555), \
                ...
                patch("fivetran_connector_sdk.atexit.register", side_effect=lambda _: events.append("crash_report")), \
    ...
    @patch("fivetran_connector_sdk.check_newer_version")
    @patch("fivetran_connector_sdk.connector_helper.tester_root_dir_helper", return_value="/tmp/tester")
    @patch("fivetran_connector_sdk.connector_helper.get_os_arch_suffix", return_value="osx-arm64")
    @patch("fivetran_connector_sdk.connector_helper.java_exe_helper", return_value="/tmp/tester/java")
    ...
    @patch("os.chmod")
    @patch("fivetran_connector_sdk.Connector.run")
    def test_debug_install_needed(self, mock_run, mock_chmod, mock_stat, mock_delete, mock_zip, mock_openfile, mock_rq,
                                  mock_makedirs, mock_isfile, mock_java, mock_arch, mock_dir, mock_check, mock_tester,
                                  mock_port, mock_exit, mock_validate_req, mock_validate_toml, mock_log):
        mock_server = MagicMock()
        mock_run.return_value = mock_server
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("fivetran_connector_sdk.Connector.run")
    def test_debug_download_fails(self, mock_run, mock_openfile, mock_rq, mock_makedirs, mock_isfile, mock_java,
                                  mock_arch, mock_dir, mock_check, mock_port, mock_exit, mock_validate, mock_log):
    ...
    @patch("fivetran_connector_sdk.Connector.run")
    def test_debug_unzip_fails(self, mock_run, mock_rmtree, mock_zip, mock_openfile, mock_rq, mock_makedirs,
                               mock_isfile, mock_java, mock_arch, mock_dir, mock_check, mock_port, mock_exit,
                               mock_validate, mock_log):
        mock_rq.return_value.ok = True
        mock_rq.return_value.headers = {}
        mock_rq.return_value.iter_content.return_value = [b"zip"]
        mock_rq.return_value.__enter__.return_value = mock_rq.return_value
    ...
    @patch("fivetran_connector_sdk.ensure_tester_installed", return_value=("/tmp/tester/java", "/tmp/tester"))
    @patch("fivetran_connector_sdk.Connector.run")
    def test_debug_run_tester_exception(self, mock_run, mock_ensure, mock_check, mock_tester, mock_port, mock_exit,
                                        mock_validate, mock_log, mock_update_base_url):
        mock_server = MagicMock()
        mock_run.return_value = mock_server
        connector = Connector(update=MagicMock())
        connector.state = {}
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_new_connection_success(self, mock_version_check, mock_log, mock_check, mock_validate_req, mock_group,
                                           mock_conn_id, mock_upload, mock_create, mock_handle_response):
        ...
        path, key, group, conn, hdid, conf, confpath = "path", "key", "group", "conn", "hdid", {"foo": "bar"}, "conf.json"
        ...
        mock_handle_response.assert_called_once_with(mock_resp, "pkg_123", key, HTTPStatus.CREATED.value, is_new_connection=True)
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_new_connection_setup_tests_fail(self, mock_version_check, mock_log, mock_check, mock_validate_req,
                                                    mock_group, mock_conn_id, mock_upload, mock_create, mock_handle_response):
        from http import HTTPStatus
        mock_resp = MagicMock()
        mock_resp.ok = True
        mock_resp.status_code = 201
        ...
        mock_create.assert_called_once()
        mock_handle_response.assert_called_once_with(mock_resp, "pkg_456", "key", HTTPStatus.CREATED.value, is_new_connection=True)
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_new_connection_create_fails(self, mock_version_check, mock_log, mock_check, mock_validate_req,
                                                mock_group, mock_conn_id, mock_upload, mock_create, mock_handle_response):
        from http import HTTPStatus
        mock_resp = MagicMock()
        mock_resp.ok = False
        ...
        mock_upload.assert_called_once()
        mock_create.assert_called_once()
        mock_handle_response.assert_called_once_with(mock_resp, "pkg_789", "key", HTTPStatus.CREATED.value, is_new_connection=True)
    ...
    def test_deploy_update_connection_with_configuration_uses_one_confirmation(
            self, mock_confirmation, mock_version_check, mock_log, mock_check, mock_validate_req, mock_group,
            mock_conn_id, mock_upload, mock_update, mock_handle_response):
        from http import HTTPStatus
        mock_resp = MagicMock()
        mock_resp.ok = True
        ...
        mock_log.assert_any_call("updating connection conn in group gname", log_icon=Logging.LogIcon.STEP)
        mock_handle_response.assert_called_once_with(mock_resp, "pkg_999", key, HTTPStatus.OK.value, is_new_connection=False, connection_id="cid")
    ...
    def test_deploy_update_connection_cancels_when_configuration_confirmation_is_rejected(
            self, mock_confirmation, mock_version_check, mock_log, mock_check, mock_validate_req, mock_group,
            mock_conn_id, mock_upload, mock_update, mock_handle_response, mock_exit):
    ...
    @patch("fivetran_connector_sdk.check_dict")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_update_connection_wrong_service(self, mock_version_check, mock_check, mock_validate_req, mock_group,
                                                    mock_conn_id, mock_log, mock_exit):
    ...
    @patch("fivetran_connector_sdk.check_dict")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_update_connection_with_naming(self, mock_version_check, mock_check, mock_validate_req, mock_group,
                                                    mock_conn_id, mock_log, mock_exit):
        self.connector.deploy("path", "key", "group", "conn", "hdid", {"foo": "bar"}, naming="SOURCE_NAMING")
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_logs_python_version_not_specified(self, mock_version_check, mock_log, mock_check, mock_validate_req, mock_group,
                                                      mock_conn_id, mock_upload, mock_create, mock_handle_response):
    ...
    def test_deploy_existing_connection_does_not_log_python_version_not_specified(
            self, mock_version_check, mock_log, mock_check, mock_validate_req, mock_group,
            mock_conn_id, mock_upload, mock_update, mock_handle_response):
        self.connector.deploy("path", "key", "group", "conn", "hdid", prompt_mode=PromptMode.FORCE, configuration={})

        python_version_calls = [
            call for call in mock_log.call_args_list
            if call.args and "python version not specified" in call.args[0]
        ]
        self.assertEqual(len(python_version_calls), 0,
    ...
    @patch("fivetran_connector_sdk.__init__.get_configuration", return_value=({"test": "config"}, "config.json"))
    ...
    @patch("fivetran_connector_sdk.__init__.find_connector_object")
    @patch("fivetran_connector_sdk.__init__.print_library_log")
    def test_main_debug_subprocess_error(self, mock_log, mock_find, _mock_get_state, _mock_get_configuration):
        """Test main() debug command with subprocess.CalledProcessError"""
        import subprocess
        mock_connector = MagicMock()
        mock_connector.debug = MagicMock(side_effect=subprocess.CalledProcessError(returncode=42, cmd="test"))
        mock_find.return_value = mock_connector
        test_args = ["fivetran", "debug", "/test/path"]
        ...
        with patch.object(sys, "argv", test_args):
            ...
            mock_log.assert_any_call("connector tester failed with exit code: 42", level=Logging.Level.SEVERE, log_icon=Logging.LogIcon.FAILURE)

    @patch("fivetran_connector_sdk.__init__.get_configuration", return_value=({}, None))
    @patch("fivetran_connector_sdk.__init__.get_state", return_value={})
    @patch("fivetran_connector_sdk.__init__.find_connector_object")
    @patch("fivetran_connector_sdk.__init__.print_library_log")
    def test_main_debug_generic_exception(self, mock_log, mock_find, _mock_get_state, _mock_get_configuration):
        ...
        with patch.object(sys, "argv", test_args):
            ...
            # Verify error was logged
            mock_log.assert_any_call("debug run failed error: Something went wrong", level=Logging.Level.SEVERE, log_icon=Logging.LogIcon.FAILURE)

    @patch("fivetran_connector_sdk.__init__.get_configuration", return_value=({}, None))
    @patch("fivetran_connector_sdk.__init__.get_state", return_value={})
    @patch("fivetran_connector_sdk.__init__.find_connector_object")
    def test_main_debug_env_vars_set_and_cleaned(self, mock_find, _mock_get_state, _mock_get_configuration):
        ...
        def capture_env(*args, **kwargs):
            env_vars_during_call['FIVETRAN_CONNECTION_ID'] = os.environ.get('FIVETRAN_CONNECTION_ID')
            env_vars_during_call['FIVETRAN_DEPLOYMENT_MODEL'] = os.environ.get('FIVETRAN_DEPLOYMENT_MODEL')
            env_vars_during_call['FIVETRAN_GROUP_ID'] = os.environ.get('FIVETRAN_GROUP_ID')
            env_vars_during_call['FIVETRAN_CONNECTION_NAME'] = os.environ.get('FIVETRAN_CONNECTION_NAME')
        ...
        with patch.object(sys, "argv", test_args):
            ...
            self.assertEqual(env_vars_during_call['FIVETRAN_CONNECTION_NAME'], 'test_connection_name')
    ...
    @patch("fivetran_connector_sdk.__init__.find_connector_object")
    @patch("fivetran_connector_sdk.__init__.print_library_log")
    def test_main_debug_env_vars_cleaned_on_exception(self, _mock_log, mock_find, _mock_get_state, _mock_get_configuration):
    ...
    def test_package_command_success(self, mock_log, mock_validate, mock_create, mock_exit):
        ...
        mock_log.assert_any_call("package created at: /path/to/package.zip", log_icon=Logging.LogIcon.SUCCESS)
        mock_exit.assert_called_once_with(0)
    ...
    @patch("fivetran_connector_sdk.validate_requirements_file")
    @patch("fivetran_connector_sdk.print_library_log")
    def test_package_command_with_non_interactive(self, mock_log, mock_validate, mock_create, mock_exit):
        ...
        mock_validate.assert_called_once_with("/test/project", True, __version__, PromptMode.DEFAULT_ANSWER)
        mock_create.assert_called_once_with("/test/project", None)
        mock_exit.assert_called_once_with(0)
    ...
    @patch("fivetran_connector_sdk.validate_requirements_file")
    @patch("fivetran_connector_sdk.print_library_log")
    def test_package_command_with_pyproject_toml(self, mock_log, mock_validate_req, mock_validate_toml, mock_create, mock_exit, mock_exists):
    ...
    @patch("fivetran_connector_sdk.validate_requirements_file")
    @patch("fivetran_connector_sdk.print_library_log")
    def test_package_command_with_non_interactive_and_pyproject_toml(self, mock_log, mock_validate_req, mock_validate_toml,
                                                                     mock_create, mock_exit, mock_exists):
        ...
        mock_validate_toml.assert_called_once_with("/test/project", True, PromptMode.DEFAULT_ANSWER)
        mock_validate_req.assert_not_called()
        mock_create.assert_called_once_with("/test/project", None)
        mock_exit.assert_called_once_with(0)
...
class TestConfigurationFormHandler(unittest.TestCase):
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    def test_configuration_form_debug_stores_crash_report_without_traceback_in_error(self, _mock_log):
        connector = Connector(update=MagicMock())
        connector.configuration_form_method = MagicMock(side_effect=ValueError("bad form"))
        connector.project_path = "/project"
        ...
        constants.DEBUGGING = True
        try:
            with patch("fivetran_connector_sdk.build_debug_crash_report", return_value="FORM REPORT") as mock_report:
                with self.assertRaises(RuntimeError) as cm:
                    connector.ConfigurationForm(MagicMock(), MagicMock())
        finally:
            constants.DEBUGGING = original_debugging
...
class TestTestHandler(unittest.TestCase):
    ...
    def test_test_debug_stores_crash_report_without_traceback_in_error(self, _mock_log):
        ...
        constants.DEBUGGING = True
        try:
            with patch("fivetran_connector_sdk.build_debug_crash_report", return_value="TEST REPORT") as mock_report:
                with self.assertRaises(RuntimeError) as cm:
                    connector.Test(self._make_request(name="failing_test"), MagicMock())
        finally:
            constants.DEBUGGING = original_debugging
...
class TestGenerateConfiguration(unittest.TestCase):
    ...
    def test_exits_when_no_configuration_form_method(self, mock_log):
        ...
        self.assertEqual(cm.exception.code, 1)
        self.assertTrue(any(
            "Your connector does not implement the configuration_form() method" in str(call.args[0])
    ...
    def test_prompts_and_continues_when_configuration_json_exists_and_user_confirms(
            self, _mock_log, _mock_exists, _mock_input, _mock_check, _mock_install,
            _mock_port, _mock_tester, mock_run):
        mock_server = MagicMock()
        mock_run.return_value = mock_server
        connector = self._make_connector_with_form()
        connector.generate_configuration("/proj")
    ...
    @patch("os.path.exists", return_value=True)
    @patch("fivetran_connector_sdk.print_library_log")
    def test_exits_when_user_declines_override(self, mock_log, _mock_exists, _mock_input, _mock_check):
        connector = self._make_connector_with_form()
        with self.assertRaises(SystemExit) as cm:
            connector.generate_configuration("/proj")
        self.assertEqual(cm.exception.code, 0)
    ...
    def test_skips_prompt_when_configuration_json_not_exists(
            self, _mock_log, _mock_exists, _mock_check, _mock_install,
            _mock_port, _mock_tester, mock_run):
        mock_server = MagicMock()
        mock_run.return_value = mock_server
    ...
    @patch("fivetran_connector_sdk.Connector.run")
    @patch("fivetran_connector_sdk.run_configuration_tester", side_effect=Exception("tester error"))
    @patch("fivetran_connector_sdk.get_available_port", return_value=50051)
    @patch("fivetran_connector_sdk.ensure_tester_installed", return_value=("/java", "/tester"))
    @patch("fivetran_connector_sdk.check_newer_version")
    @patch("os.path.exists", return_value=False)
    ...
    def test_does_not_print_traceback_when_configuration_tester_returns_non_zero(
            self, mock_print, _mock_exists, _mock_check, _mock_install,
            _mock_port, mock_tester, mock_run):
        import subprocess
        mock_server = MagicMock()
        mock_run.return_value = mock_server
        ...
        self.assertFalse(any("Traceback (most recent call last)" in message for message in printed_messages))
        mock_server.stop.assert_called_once_with(grace=2.0)
...
class TestMainConfigurationCommand(unittest.TestCase):
    ...
    def test_configuration_command_calls_generate_configuration(self, mock_find):
        ...
        with patch.object(sys, "argv", ["fivetran", "configuration", "/proj"]):
            ...
            try:
                ...
                pass
        mock_connector.generate_configuration.assert_called_once_with("/proj", run_tests=False, disable_encryption=False)
    ...
    def test_configuration_command_subprocess_error(self, mock_log, mock_find):
        import subprocess
        mock_connector = MagicMock()
        mock_connector.generate_configuration.side_effect = subprocess.CalledProcessError(returncode=42, cmd="test")
        mock_find.return_value = mock_connector
        with patch.object(sys, "argv", ["fivetran", "configuration", "/proj"]):
            from fivetran_connector_sdk.__init__ import main
            with self.assertRaises(SystemExit) as cm:
    ...
    def test_configuration_command_generic_exception(self, mock_log, mock_find):
        ...
        with patch.object(sys, "argv", ["fivetran", "configuration", "/proj"]):
            ...
            with self.assertRaises(SystemExit) as cm:
                main()
        self.assertEqual(cm.exception.code, 1)
        mock_log.assert_any_call("configuration command failed error: something failed", level=Logging.Level.SEVERE, log_icon=Logging.LogIcon.FAILURE)

    @patch("fivetran_connector_sdk.__init__.find_connector_object")
    def test_configuration_command_with_disable_encryption_flag(self, mock_find):
        mock_connector = MagicMock()
        mock_find.return_value = mock_connector
        with patch.object(sys, "argv", ["fivetran", "configuration", "/proj", "--disable-encryption"]):
            from fivetran_connector_sdk.__init__ import main
            try:
                main()
            except SystemExit:
                pass
        mock_connector.generate_configuration.assert_called_once_with("/proj", run_tests=False, disable_encryption=True)

    @patch("fivetran_connector_sdk.__init__.find_connector_object")
    def test_configuration_command_with_test_and_disable_encryption(self, mock_find):
        mock_connector = MagicMock()
        mock_find.return_value = mock_connector
        with patch.object(sys, "argv", ["fivetran", "configuration", "/proj", "--test", "--disable-encryption"]):
            from fivetran_connector_sdk.__init__ import main
            try:
                main()
            except SystemExit:
                pass
        mock_connector.generate_configuration.assert_called_once_with("/proj", run_tests=True, disable_encryption=True)
...
class TestDeployWithProxy(unittest.TestCase):
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_new_connection_with_proxy_id(self, mock_version, mock_log, mock_check, mock_validate,
                                                  mock_group, mock_conn, mock_upload, mock_create, mock_handle):
        ...
        with patch("fivetran_connector_sdk.validate_proxy_configuration", return_value="host"):
            self.connector.deploy(
                "path", "key", "group", "conn", None,
                config, "conf.json", python_version="3.12",
                proxy_id="proxy-123", proxy_host_config_key="host",
                prompt_mode=PromptMode.YES,
            )
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_update_clears_proxy_when_not_provided(self, mock_version, mock_log, mock_check, mock_validate,
                                                           mock_group, mock_conn, mock_upload, mock_update, mock_handle):
        ...
        self.connector.deploy(
            "path", "key", "group", "conn", None,
            prompt_mode=PromptMode.YES, proxy_id=None,
        )
    ...
    def test_deploy_exits_when_proxy_host_config_key_without_proxy_id(
        ...
        with self.assertRaises(SystemExit):
            self.connector.deploy(
                "path", "key", "group", "conn", None,
                prompt_mode=PromptMode.YES, proxy_host_config_key="host",
            )
    ...
    def test_deploy_exits_when_proxy_id_and_hd_agent_id_both_provided(
        ...
        with self.assertRaises(SystemExit):
            self.connector.deploy(
                "path", "key", "group", "conn", hd_agent_id="hd-agent-123",
                prompt_mode=PromptMode.YES, proxy_id="proxy-123",
            )

        calls = [str(c) for c in mock_log.call_args_list]
        self.assertTrue(any("Proxy Agent is not supported in Hybrid Deployment connections" in c for c in calls))
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_hosts_list_preserved_in_secrets_list(self, mock_version, mock_log, mock_validate,
                                                          mock_group, mock_conn, mock_upload, mock_create, mock_handle):
        ...
        self.connector.deploy(
            "path", "key", "group", "conn", None,
            config, "conf.json", python_version="3.12",
            proxy_id="proxy-123", proxy_host_config_key=None,
            prompt_mode=PromptMode.YES,
        )

        mock_create.assert_called_once()
        call_args = mock_create.call_args
        connection_config = call_args.args[2] if len(call_args.args) > 2 else call_args.kwargs.get("config")
    ...
    @patch("fivetran_connector_sdk.print_library_log")
    @patch("fivetran_connector_sdk.check_newer_version")
    def test_deploy_auto_detected_host_key_set_in_connection_config(self, mock_version, mock_log, mock_validate,
                                                                     mock_group, mock_conn, mock_upload, mock_create, mock_handle):
