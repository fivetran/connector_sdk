...
from fivetran_connector_sdk.constants import CONFIGURATION_FORM_FILENAME, ROOT_LOCATION, TESTER_LOCATION
import unittest
...
class TestConnectorHelper(unittest.TestCase):
    ...
    @patch("fivetran_connector_sdk.connector_helper.copy_requirements_file_to_tmp_requirements_file")
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open, read_data="package==1.0.0\n")
    def test_validate_requirements_file_success(
        self, mock_open_file, mock_exists, mock_isdir, mock_listdir, mock_subprocess_run,
        mock_copy, mock_fetch, mock_delete
    ):
    ...
    def test_validate_requirements_file_retries_and_fails(
            self, mock_subprocess_run, mock_open_file, mock_exists, mock_isdir, mock_listdir,
            mock_copy, mock_fetch, mock_delete, mock_print_log, mock_sleep
    ):
        ...
        self.assertTrue(any("attempt" in str(call) for call in [args[0][0] for args in mock_print_log.call_args_list]))
        self.assertTrue(
            any("retrying after" in str(call) for call in [args[0][0] for args in mock_print_log.call_args_list]))
        self.assertTrue(
            any("pipreqs failed after" in str(call) for call in [args[0][0] for args in mock_print_log.call_args_list]))
        self.assertTrue(any("skipping requirements.txt validation" in str(call) for call in
                            [args[0][0] for args in mock_print_log.call_args_list]))
    ...
    def test_version_mismatch_deps_prompt_y(
            self, mock_input, mock_open_file, mock_exists, mock_isdir, mock_listdir, mock_subprocess_run,
            mock_copy, mock_fetch, mock_delete
    ):
    ...
    def test_version_mismatch_deps_prompt_n(
        self, mock_log, mock_input, mock_open_file, mock_exists, mock_isdir, mock_listdir, mock_subprocess_run,
        mock_copy, mock_fetch, mock_delete
    ):
        mock_exists.return_value = True
        mock_isdir.return_value = False
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open, read_data="package==1.0.0\n")
    @patch("builtins.input")
    def test_empty_pipreqs_response(self, mock_input, mock_open_file, mock_exists, mock_isdir, mock_listdir,
                                    mock_subprocess_run, mock_copy, mock_fetch, mock_delete, mock_print):
        ...
        validate_requirements_file("dummy_path", is_deploy=True, version="1.0.0")
        warning_calls = [call for call in mock_print.call_args_list if len(call[0]) > 0 and "package" in str(call[0][0])]
        self.assertTrue(len(warning_calls) > 0, "Expected warning message containing 'package' to be printed")
    ...
    def test_unused_deps_logs_without_prompting_or_updating(
            self, mock_input, mock_print, mock_open_file, mock_exists, mock_isdir, mock_listdir, mock_subprocess_run,
            mock_copy, mock_fetch, mock_delete
    ):
        ...
        validate_requirements_file("dummy_path", is_deploy=True, version="1.0.0")
        mock_input.assert_not_called()
        warning_calls = [call for call in mock_print.call_args_list if len(call[0]) > 0 and "unused" in str(call[0][0])]
        self.assertTrue(len(warning_calls) > 0, "Expected warning message containing 'unused' to be printed")
        mock_open_file().write.assert_not_called()
    ...
    def test_missing_deps_prompt_empty_input(
        self, mock_resolve_confirmation, mock_open_file, mock_exists, mock_isdir, mock_listdir, mock_subprocess_run,
        mock_copy, mock_fetch, mock_delete, mock_log
    ):
    ...
    def test_new_version_available(
            self, mock_time, mock_open_file, mock_rq_get, mock_isfile, mock_isdir, mock_makedirs, mock_log
    ):
        ...
        mock_log.assert_any_call("fivetran-connector-sdk 2.0.0 is available. (a newer release exists)", log_icon=Logging.LogIcon.LIGHTNING, level=Logging.Level.WARNING)
        mock_log.assert_any_call("run 'pip install --upgrade fivetran-connector-sdk' to update", log_icon=Logging.LogIcon.LIGHTNING, level=Logging.Level.WARNING)
    ...
    def test_no_new_version(
        ...
        self.assertFalse(any("new release" in str(call) for call in [args[0][0] for args in mock_log.call_args_list]))
    ...
    def test_minor_version_no_false_positive(
        ...
        self.assertFalse(any("is available" in str(call) for call in [args[0][0] for args in mock_log.call_args_list]))
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open, read_data="99999")
    @patch("fivetran_connector_sdk.connector_helper.time.time", return_value=100000)
    ...
    def test_request_fails_and_retries(
            self, mock_sleep, mock_time, mock_open_file, mock_rq_get, mock_isfile, mock_isdir, mock_makedirs, mock_log
    ):
        from fivetran_connector_sdk.connector_helper import check_newer_version, MAX_RETRIES
        check_newer_version("1.0.0")
        # Should log a warning for each retry
        self.assertTrue(any("unable to check for a newer version" in str(call) for call in
                            [args[0][0] for args in mock_log.call_args_list]))
    ...
    def test_exit_call(self, mock_log):
        ...
        with patch("builtins.open", mock_open(read_data=code)):
            with patch("os.path.join", return_value="dummy_path/connector.py"):
                exit_check("dummy_path")
        self.assertTrue(
            any("avoid using exit()" in str(call) for call in [args[0][0] for args in mock_log.call_args_list]))
    ...
    def test_os__exit_call(self, mock_log):
        ...
        with patch("builtins.open", mock_open(read_data=code)):
            with patch("os.path.join", return_value="dummy_path/connector.py"):
                exit_check("dummy_path")
        self.assertTrue(
            any("avoid using sys.exit()" in str(call) for call in [args[0][0] for args in mock_log.call_args_list]))
    ...
    def test_syntax_error(self, mock_log):
        ...
        with patch("builtins.open", mock_open(read_data=code)):
            with patch("os.path.join", return_value="dummy_path/connector.py"):
                exit_check("dummy_path")
        self.assertTrue(any("SyntaxError" in str(call) for call in [args[0][0] for args in mock_log.call_args_list]))
    ...
    @patch("os.path.exists", return_value=True)
    @patch("builtins.open", new_callable=mock_open, read_data="pkg1==1.0.0\npkg2>=2.0.0\npkg3<=3.0.0\n")
    ...
    def test_invalid_requirement_format(self, mock_log, mock_file, mock_exists):
        with patch("fivetran_connector_sdk.connector_helper.fetch_requirements_from_file", return_value=["badline"]):
    ...
    @patch("fivetran_connector_sdk.connector_helper.copy_requirements_file_to_tmp_requirements_file")
    @patch("fivetran_connector_sdk.connector_helper.load_or_add_requirements_file", return_value={})
    @patch("fivetran_connector_sdk.connector_helper.os.path.exists", return_value=False)
    def test_validate_requirements_file_no_warning_when_no_deps(
        ...
        warning_calls = [
            ...
            if c.args == ("`requirements.txt` file not found in your project folder", Logging.Level.WARNING)
        ]
    ...
    def test_validate_requirements_file_no_validation_log_on_deploy_when_no_deps(
        ...
        validation_calls = [c for c in mock_log.call_args_list if "Validation of" in str(c)]
        assert validation_calls == [], "Validation completed must not fire on deploy when no deps detected"
    ...
    def test_process_columns_all_types(self, mock_pb2):
        type_map = {
            "BOOLEAN": 1, "SHORT": 2, "INT": 3, "LONG": 4, "FLOAT": 6, "DOUBLE": 7,
            "NAIVE_DATE": 8, "NAIVE_DATETIME": 9, "UTC_DATETIME": 10, "BINARY": 11,
            "XML": 12, "STRING": 13, "JSON": 14, "DECIMAL": 15
        }
        ...
        error_message = ("DECIMAL data type missing precision and scale. "
        ...
        with self.assertRaises(ValueError) as excinfo:
            entry = {
                "columns": {
                    "foo": object()  # Passing an unsupported type (not str or dict)
                }
            }
            process_columns({}, entry)
    ...
    @patch("os.path.join", return_value="dummy_path/config.json")
    @patch("os.path.isfile", return_value=True)
    @patch("builtins.open", new_callable=mock_open, read_data='{"production_base_url": "http://new.url"}')
    @patch("json.load", return_value={"production_base_url": "http://new.url"})
    @patch("fivetran_connector_sdk.print_library_log")
    def test_config_file_with_base_url(self, mock_log, mock_json, mock_open_file, mock_isfile, mock_join):
        from fivetran_connector_sdk import constants
        from fivetran_connector_sdk import update_base_url_if_required
        update_base_url_if_required()
    ...
    def test_copy_requirements_file_to_tmp_requirements_file(self, mock_fetch, mock_open_file, mock_exists):
        from fivetran_connector_sdk.connector_helper import copy_requirements_file_to_tmp_requirements_file
    ...
    @patch("fivetran_connector_sdk.connector_helper.fetch_requirements_from_file")
    @patch("builtins.open")
    def test_copy_requirements_file_to_tmp_requirements_file_file_not_exists(self, mock_open_file, mock_fetch,
                                                                             mock_exists):
        from fivetran_connector_sdk.connector_helper import copy_requirements_file_to_tmp_requirements_file
        copy_requirements_file_to_tmp_requirements_file("requirements.txt", "tmp_requirements.txt")
    ...
    @patch("fivetran_connector_sdk.connector_helper.constants")
    def test_update_connection_all_cases(
            self, mock_constants, mock_patch, mock_log
    ):
        from fivetran_connector_sdk.connector_helper import SETUP_TESTS_RUNNING_MESSAGE, update_connection
    ...
    @patch("fivetran_connector_sdk.connector_helper.print_library_log")
    def test_handle_failing_tests_message_and_exit(
            self, mock_log, mock_print_failing, mock_exit
    ):
        from fivetran_connector_sdk.connector_helper import handle_failing_tests_message_and_exit, Logging
        resp = MagicMock()
    ...
    def test_create_connection_success(self, mock_constants, mock_post, mock_log, mock_user_agent):
        from fivetran_connector_sdk.connector_helper import SETUP_TESTS_RUNNING_MESSAGE, create_connection
        ...
        result = create_connection(deploy_key, group_id, config, hd_agent_id, package_id, FIVETRAN_NAMING)
    ...
    def test_connection_found(self, mock_get):
        ...
        mock_resp.json.return_value = {
            "data": {
                "items": [{"id": "conn-123", "service": "connector_sdk"}]
            }
        }
    ...
    def test_create_upload_file_returns_zip_path(self, mock_zip_folder, mock_log):
        from fivetran_connector_sdk.connector_helper import create_package, CONFIGURATION_FORM_FILENAME
        from fivetran_connector_sdk.logger import Logging
    ...
    @patch("fivetran_connector_sdk.connector_helper.ZipFile")
    @patch("fivetran_connector_sdk.connector_helper.dir_walker")
    def test_missing_connector_file(self, mock_dir_walker, mock_zipfile, mock_join, mock_log, mock_exit):
        mock_dir_walker.return_value = [
            ("proj", ["foo.txt"]),
        ]
        mock_zip = MagicMock()
        ...
        mock_log.assert_any_call("connector.py not found in the project root\nthis file is required to start a sync and must be named in lowercase", Logging.Level.SEVERE)
        mock_exit.assert_called_once_with(1)
    ...
    @patch("fivetran_connector_sdk.connector_helper.ZipFile")
    @patch("fivetran_connector_sdk.connector_helper.dir_walker")
    def test_custom_drivers_without_install_script(self, mock_dir_walker, mock_zipfile, mock_join, mock_log, mock_exit):
        mock_dir_walker.return_value = [
            ("proj", ["connector.py"]),
    ...
    @patch("fivetran_connector_sdk.connector_helper.dir_walker")
    @patch("os.makedirs")
    def test_zip_folder_dynamic_filename_from_project_name(self, mock_makedirs, mock_dir_walker, mock_zipfile):
        ...
        result = zip_folder("/path/to/my_connector", extra_files={CONFIGURATION_FORM_FILENAME: b""})
    ...
    def test_zip_folder_handles_trailing_slash(self, mock_makedirs, mock_dir_walker, mock_zipfile):
        ...
        result = zip_folder("/path/to/test_project/", extra_files={CONFIGURATION_FORM_FILENAME: b""})
    ...
    @patch("fivetran_connector_sdk.connector_helper.dir_walker")
    @patch("os.makedirs")
    def test_zip_folder_fallback_to_upload_filename(self, mock_makedirs, mock_dir_walker, mock_zipfile):
    ...
    @patch("fivetran_connector_sdk.connector_helper.dir_walker")
    @patch("os.makedirs")
    def test_zip_folder_resolves_relative_paths(self, mock_makedirs, mock_dir_walker, mock_zipfile):
    ...
    def test_drivers_folder_uppercase_D_not_treated_as_drivers(
        ...
        mock_zip.write.assert_has_calls(
            [
                call(os.path.join("proj", "connector.py"), os.path.relpath(os.path.join("proj", "connector.py"), "proj")),
                call(
    ...
    def test_dir_walker_basic(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
    ...
    def test_upload_failure(self, mock_file, mock_post, mock_log, mock_os_arch):
        ...
        self.assertIsNone(result)
        mock_log.assert_any_call(
            "failed to upload the package error: Bad Request: File not valid", level=Logging.Level.SEVERE, log_icon=Logging.LogIcon.FAILURE
        )
    ...
    def test_cleanup_success(self, mock_constants, mock_delete, mock_log, mock_user_agent):
        ...
        mock_log.assert_any_call("cleaning up orphaned package: pkg_123", log_icon=Logging.LogIcon.STEP)
    ...
    def test_no_groups_exits(self, mock_exit, mock_log):
        ...
        if not groups:
            ...
            import os
            sys.exit(1)
        mock_log.assert_called_once_with("No destinations defined in the account", Logging.Level.SEVERE)
        mock_exit.assert_called_once_with(1)
    ...
    @patch("fivetran_connector_sdk.connector_helper._maybe_colorize_jar_output", side_effect=lambda x: x)
    ...
    @patch("fivetran_connector_sdk.connector_helper.os.path.join", side_effect=lambda *args: "/".join(args))
    def test_run_tester_success(self, mock_join, mock_mkdir, mock_subprocess, mock_process_stream, mock_colorize):
    ...
    @patch("fivetran_connector_sdk.connector_helper.os.path.join", side_effect=lambda *args: "/".join(args))
    def test_run_tester_nonzero_exit(self, mock_join, mock_mkdir, mock_subprocess, mock_process_stream, mock_colorize):
        ...
        with patch("fivetran_connector_sdk.connector_helper.subprocess.CalledProcessError",
    ...
    @patch("fivetran_connector_sdk.connector_helper.os.mkdir", side_effect=FileExistsError)
    @patch("fivetran_connector_sdk.connector_helper.os.path.join", side_effect=lambda *args: "/".join(args))
    def test_run_tester_redacts_configuration_on_error(self, mock_join, mock_mkdir, mock_subprocess, mock_process_stream, mock_colorize):
        import subprocess as real_subprocess
        from fivetran_connector_sdk.connector_helper import run_tester
        ...
        redacted_config_expected = json.dumps({"username": REDACTED_VALUE, "password": REDACTED_VALUE, "api_key": REDACTED_VALUE})
    ...
    @patch("fivetran_connector_sdk.connector_helper.os.mkdir", side_effect=FileExistsError)
    @patch("fivetran_connector_sdk.connector_helper.os.path.join", side_effect=lambda *args: "/".join(args))
    def test_run_tester_redacts_configuration_with_fewer_keys(self, mock_join, mock_mkdir, mock_subprocess, mock_process_stream, mock_colorize):
        import subprocess as real_subprocess
        from fivetran_connector_sdk.connector_helper import run_tester
        import json
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_gitignore_preserves_drivers_behavior(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        from fivetran_connector_sdk.connector_helper import dir_walker
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_no_gitignore_backwards_compatible(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        from fivetran_connector_sdk.connector_helper import dir_walker
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_includes_gitignore_despite_dot_pattern(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        ...
        self.assertIn(".gitignore", root_files, ".gitignore should be included via ALWAYS_INCLUDED_FILES")
        ...
        self.assertIn(".gitignore", src_files, ".gitignore in subdirectory should also be included via ALWAYS_INCLUDED_FILES")
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_excludes_virtual_environments(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        from fivetran_connector_sdk.connector_helper import dir_walker
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_directory_negation_patterns(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        ...
        self.assertIsNotNone(foo_result, "Directory 'foo/' should be descended into when negation patterns exist")
        _, foo_files = foo_result
        self.assertIn("bar.py", foo_files, "bar.py should be included due to negation pattern '!foo/bar.py'")
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_gitignore_negation_does_not_override_venv_exclusion(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        ...
        self.assertIsNone(lib_result, "Virtual environment 'lib/' should be excluded even with .gitignore negation pattern '!lib/'")

    @patch("os.listdir")
    @patch("os.path.isdir")
    @patch("os.path.join", side_effect=lambda a, b: f"{a}\\{b}")  # Windows-style join
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}\\", ""))  # Windows-style relpath
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_windows_path_separators(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        ...
        self.assertNotIn("test.txt", root_files, "test.txt should be excluded by .gitignore pattern")
        ...
        self.assertIsNone(build_result, "build/ should be excluded by .gitignore pattern even with Windows backslash paths")
    ...
    @patch('os.listdir')
    @patch('os.path.isdir')
    def test_dir_walker_hierarchical_gitignore_anchored_patterns(self, mock_isdir, mock_listdir, mock_open_file, mock_exists):
        ...
        self.assertNotIn("root_secret.py", root_files, "/root_secret.py should exclude root_secret.py at project root")
        ...
        _, src_files = src_result
        self.assertIn("app.py", src_files)
        self.assertNotIn("secret.py", src_files, "/secret.py in src/.gitignore should exclude src/secret.py")
        ...
        self.assertIn("secret.py", nested_files, "Anchored pattern /secret.py should not exclude src/nested/secret.py")
    ...
    @patch('os.listdir')
    @patch('os.path.isdir')
    def test_dir_walker_hierarchical_gitignore_unanchored_patterns(self, mock_isdir, mock_listdir, mock_open_file, mock_exists):
        ...
        self.assertEqual(root_result[0], "proj")
        _, root_files = root_result
        self.assertIn("temp.py", root_files, "temp.py at root should be included (no pattern matches it)")
        ...
        self.assertNotIn("trace.pyc", sub_files, "Unanchored pattern *.pyc should match recursively in src/sub/")
    ...
    @patch('os.listdir')
    @patch('os.path.isdir')
    def test_dir_walker_hierarchical_gitignore_negation_with_anchoring(self, mock_isdir, mock_listdir, mock_open_file, mock_exists):
        ...
        self.assertIsNotNone(src_result)
        _, sub_files = sub_result
        self.assertIn("trace.py", sub_files, "Anchored pattern /*.py should not exclude trace.py in subdirectory")
        self.assertIn("data.py", sub_files)
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_drivers_folder_with_negation_patterns(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
        ...
        self.assertNotIn("driver1.jar", drivers_files, "driver1.jar should be excluded by *.jar pattern")
        self.assertNotIn("driver2.jar", drivers_files, "driver2.jar should be excluded by *.jar pattern")
        self.assertIn("important.jar", drivers_files, "important.jar should be re-included by !important.jar negation")
    ...
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    @patch("fivetran_connector_sdk.connector_helper.print_library_log")
    def test_dir_walker_invalid_pattern_exits_with_error(self, mock_log, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
    ...
    @patch("os.path.relpath", side_effect=lambda p, s: p.replace(f"{s}/", ""))
    @patch("fivetran_connector_sdk.connector_helper.load_gitignore")
    def test_dir_walker_valid_patterns_do_not_raise_error(self, mock_load_gitignore, mock_relpath, mock_join, mock_isdir, mock_listdir):
    ...
    @patch('fivetran_connector_sdk.connector_helper.print_library_log')
    @patch('fivetran_connector_sdk.connector_helper.config_root_dir_helper', return_value='/test/root')
    @patch('os.path.isfile')
    @patch('builtins.open', new_callable=mock_open, read_data='{"production_base_url": "http://new.url"}')
    def test_update_base_url_if_required_with_config(self, mock_file, mock_isfile, mock_root, mock_log):
        ...
        try:
            ...
            mock_log.assert_called_with("using custom production url: http://new.url", log_icon=Logging.LogIcon.INFO)
    ...
    def test_update_base_url_if_required_no_base_url_key(self, mock_file, mock_isfile, mock_root, mock_log):
    ...
    def test_handle_connection_response_new_connection_success(self, mock_failing, mock_log, mock_cleanup):
        ...
        mock_response.json.return_value = {"data": {"id": "new_conn_id", "config": {"python_version": "3.9"}, "destination_schema_names": FIVETRAN_NAMING}}

        handle_connection_response(mock_response, "pkg_123", "deploy_key", HTTPStatus.CREATED.value, is_new_connection=True)
        ...
        mock_log.assert_any_call(f"naming strategy: {FIVETRAN_NAMING}", level=Logging.Level.INFO, indent=True)
    ...
    @patch('fivetran_connector_sdk.connector_helper.print_library_log')
    @patch('fivetran_connector_sdk.connector_helper.are_setup_tests_failing', return_value=False)
    def test_handle_connection_response_existing_connection_success(self, mock_failing, mock_log, mock_cleanup):
        ...
        mock_response.json.return_value = {"data": {"config": {"python_version": "3.10"}, "destination_schema_names": FIVETRAN_NAMING}}

        handle_connection_response(mock_response, "pkg_456", "deploy_key", HTTPStatus.OK.value, is_new_connection=False, connection_id="existing_conn_id")
    ...
    def test_handle_connection_response_create_failure(self, mock_log, mock_cleanup, mock_exit):
        ...
        handle_connection_response(mock_response, "pkg_789", "deploy_key", HTTPStatus.CREATED.value, is_new_connection=True)

        mock_log.assert_any_call("failed to create connection error: Invalid config", level=Logging.Level.SEVERE, log_icon=Logging.LogIcon.FAILURE)
    ...
    def test_handle_connection_response_update_failure(self, mock_log, mock_cleanup, mock_exit):
        ...
        handle_connection_response(mock_response, "pkg_999", "deploy_key", HTTPStatus.OK.value, is_new_connection=False, connection_id="conn_id")

        mock_log.assert_any_call("failed to update connection error: Connection not found", level=Logging.Level.SEVERE, log_icon=Logging.LogIcon.FAILURE)
    ...
    def test_handle_connection_response_setup_tests_fail(self, mock_failing, mock_handle_fail, mock_exit):
        ...
        handle_connection_response(mock_response, "pkg_111", "deploy_key", HTTPStatus.CREATED.value, is_new_connection=True)

        mock_handle_fail.assert_called_once_with(mock_response, "connection created but setup tests failed")
    ...
    def test_get_connection_details_additional(self, mock_get):
        ...
        mock_response.json.return_value = {
            "data": {
                "items": [
                    {"schema": "test_conn", "id": "conn_123", "service": "connector_sdk"}
                ]
            }
        }
    ...
    def test_get_configuration_uses_configuration_json(self, mock_exists, mock_log, mock_validate):
        ...
        info_calls = [c for c in mock_log.call_args_list if "reading configuration from configuration.json found in project folder" in str(c)]
        self.assertGreater(len(info_calls), 0)
    ...
    @patch('fivetran_connector_sdk.connector_helper.print_library_log')
    @patch('os.path.exists', return_value=True)
    def test_get_configuration_cli_flag_overrides_configuration_json(self, mock_exists, mock_log, mock_validate):
    ...
    @patch('fivetran_connector_sdk.connector_helper.print_library_log')
    @patch('os.path.exists', return_value=True)
    def test_get_configuration_env_var_overrides_configuration_json(self, mock_exists, mock_log, mock_validate):
    ...
    def test_get_configuration_exits_on_invalid_file_path(self, mock_log, mock_validate):
        ...
        error_calls = [c for c in mock_log.call_args_list if "invalid configuration error" in str(c)]
...
class TestGenerateConfigurationFormBytes(unittest.TestCase):
    ...
    def test_returns_empty_file_sentinel_when_no_configuration_form_method(self):
        from fivetran_connector_sdk.connector_helper import _generate_configuration_form_bytes, CONFIGURATION_FORM_FILENAME
        self.assertEqual(_generate_configuration_form_bytes(None), {CONFIGURATION_FORM_FILENAME: b''})
    ...
    def test_exits_with_severe_log_on_exception(self, mock_log, mock_exit):
        ...
        mock_log.assert_called_once_with(
            "failed to package configuration form response: import error",
...
class TestPyprojectTomlValidation(unittest.TestCase):
    ...
    @patch("fivetran_connector_sdk.connector_helper.delete_file_if_exists")
    @patch("fivetran_connector_sdk.connector_helper.fetch_requirements_as_dict")
    @patch("fivetran_connector_sdk.connector_helper.run_pipreqs_with_retries", side_effect=RuntimeError("pipreqs failed"))
    @patch("fivetran_connector_sdk.connector_helper.parse_pyproject_dependencies")
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    def test_validate_pyproject_tmp_file_deleted_on_exception(self, mock_open_file, mock_parse,
                                                               mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.parse_pyproject_dependencies")
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    def test_validate_pyproject_no_violations(self, mock_open_file, mock_parse, mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.print_library_log")
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    def test_validate_pyproject_debug_reports_missing(self, mock_open_file, mock_log, mock_parse, mock_pipreqs, mock_fetch, mock_delete):
        ...
        self.assertTrue(any("Include the following" in str(c) and "INFO" in str(c) for c in log_calls))
    ...
    @patch("fivetran_connector_sdk.connector_helper.print_library_log")
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    def test_validate_pyproject_debug_reports_unused(self, mock_open_file, mock_log, mock_parse, mock_pipreqs, mock_fetch, mock_delete):
        ...
        with patch("builtins.input") as mock_input:
            ...
            mock_input.assert_not_called()
        log_calls = mock_log.call_args_list
        self.assertTrue(any("Remove them from pyproject.toml" in str(c) and "INFO" in str(c)
                            for c in log_calls))
    ...
    @patch("fivetran_connector_sdk.connector_helper.print_library_log")
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    def test_validate_pyproject_debug_reports_version_mismatch(self, mock_open_file, mock_log, mock_parse, mock_pipreqs, mock_fetch, mock_delete):
        ...
        with patch("builtins.input") as mock_input:
            ...
            mock_input.assert_not_called()
        log_calls = mock_log.call_args_list
        self.assertTrue(any("recommend" in str(c) and "stable version" in str(c) and "INFO" in str(c)
                            for c in log_calls))
    ...
    def test_validate_pyproject_deploy_default_continues_on_missing(self, mock_input, mock_open_file, mock_log,
                                                                     mock_parse, mock_pipreqs, mock_fetch, mock_delete):
        ...
        log_calls = mock_log.call_args_list
        self.assertTrue(any("Include the following" in str(c) and "SEVERE" in str(c) for c in log_calls))
        self.assertTrue(any("Validation of" in str(c) and "completed" in str(c) for c in log_calls))
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    @patch("builtins.input", return_value="")
    def test_validate_pyproject_deploy_logs_unused_without_prompt(self, mock_input, mock_open_file, mock_log,
                                                                  mock_parse, mock_pipreqs, mock_fetch, mock_delete):
        ...
        mock_input.assert_not_called()
        log_calls = mock_log.call_args_list
        self.assertTrue(any("Remove them from pyproject.toml" in str(c) and "WARNING" in str(c)
                            for c in log_calls))
        self.assertTrue(any("Validation of" in str(c) and "completed" in str(c) for c in log_calls))
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    @patch("builtins.input", return_value="")
    def test_validate_pyproject_deploy_default_continues_on_version_mismatch(self, mock_input, mock_open_file, mock_log,
                                                                              mock_parse, mock_pipreqs, mock_fetch, mock_delete):
        ...
        mock_input.assert_called_once()
        log_calls = mock_log.call_args_list
        self.assertTrue(any("recommend" in str(c) and "stable version" in str(c) and "WARNING" in str(c)
                            for c in log_calls))
        self.assertTrue(any("Validation of" in str(c) and "completed" in str(c) for c in log_calls))
    ...
    def test_validate_pyproject_deploy_aborts_on_n_missing(self, mock_input, mock_open_file, mock_log,
                                                            mock_parse, mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    @patch("builtins.input", return_value="n")
    def test_validate_pyproject_deploy_does_not_prompt_or_abort_on_unused(self, mock_input, mock_open_file, mock_log,
                                                                          mock_parse, mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    @patch("builtins.input", return_value="n")
    def test_validate_pyproject_deploy_aborts_on_n_version_mismatch(self, mock_input, mock_open_file, mock_log,
                                                                     mock_parse, mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    @patch("builtins.input", return_value="")
    def test_validate_pyproject_deploy_two_prompts_when_all_issues(self, mock_input, mock_open_file, mock_log,
                                                                   mock_parse, mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    @patch("builtins.input", return_value="n")
    def test_validate_pyproject_deploy_aborts_on_first_decline(self, mock_input, mock_open_file, mock_log,
                                                                mock_parse, mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.print_library_log")
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    def test_validate_pyproject_debug_no_prompt(self, mock_open_file, mock_log, mock_parse, mock_pipreqs, mock_fetch, mock_delete):
    ...
    @patch("fivetran_connector_sdk.connector_helper.print_library_log")
    @patch("fivetran_connector_sdk.connector_helper.open", new_callable=mock_open)
    def test_validate_pyproject_filters_platform_packages(self, mock_open_file, mock_log, mock_parse,
                                                           mock_pipreqs, mock_fetch, mock_delete):
...
class TestValidationWithPromptMode(unittest.TestCase):
    ...
    @patch("fivetran_connector_sdk.connector_helper.run_pipreqs_with_retries")
    @patch("fivetran_connector_sdk.connector_helper.copy_requirements_file_to_tmp_requirements_file")
    @patch("fivetran_connector_sdk.connector_helper.load_or_add_requirements_file", return_value={"package": "package==1.0.0"})
    @patch("fivetran_connector_sdk.connector_helper.os.path.exists", return_value=True)
    ...
    def test_validate_requirements_with_yes_mode_auto_answers(
            self, mock_open_file, mock_resolve, mock_exists, mock_load, mock_copy, mock_pipreqs, mock_fetch, mock_delete):
...
class TestValidateProxyConfiguration(unittest.TestCase):
    ...
    def test_no_proxy_id_returns_none(self):
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        result = validate_proxy_configuration({"host": "db:5432"}, proxy_id=None, proxy_host_config_key=None)
        self.assertIsNone(result)

    def test_proxy_id_with_explicit_key(self):
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        config = {"db_host": "mydb.example.com:5432"}
        result = validate_proxy_configuration(config, proxy_id="p-123", proxy_host_config_key="db_host")
    ...
    def test_explicit_key_takes_priority_over_default_host(self):
        ...
        result = validate_proxy_configuration(config, proxy_id="p-123", proxy_host_config_key="custom_key")
    ...
    def test_proxy_id_no_host_or_hosts_exits(self, mock_log, mock_exit):
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        validate_proxy_configuration({"other_key": "value:1234"}, proxy_id="p-123", proxy_host_config_key=None)
        mock_exit.assert_called_once_with(1)
    ...
    def test_null_value_for_host_key_returns_host(self):
        ...
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        result = validate_proxy_configuration({"host": None}, proxy_id="p-123", proxy_host_config_key=None)
        self.assertEqual(result, "host")

    def test_empty_list_for_hosts_key_returns_hosts(self):
        """Value emptiness is not validated; presence of key is sufficient."""
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        result = validate_proxy_configuration({"hosts": []}, proxy_id="p-123", proxy_host_config_key=None)
        self.assertEqual(result, "hosts")

    def test_explicit_key_with_empty_value_returns_key(self):
        """Explicit --proxy-host-config-key does not validate value emptiness."""
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        result = validate_proxy_configuration({"db_host": ""}, proxy_id="p-123", proxy_host_config_key="db_host")
        self.assertEqual(result, "db_host")

    def test_key_matching_is_case_sensitive(self):
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        config = {"HOST": "db.example.com:5432"}
        with patch("fivetran_connector_sdk.connector_helper.sys.exit") as mock_exit, \
             patch("fivetran_connector_sdk.connector_helper.print_library_log"):
    ...
    def test_auto_detect_host_wins_over_hosts_when_both_present(self):
        ...
        from fivetran_connector_sdk.connector_helper import validate_proxy_configuration
        result = validate_proxy_configuration({"host": "h1:5432", "hosts": ["h2:5433"]}, proxy_id="p-123", proxy_host_config_key=None)
        self.assertEqual(result, "host")
...
class TestUpdateConnectionWithProxy(unittest.TestCase):
    ...
    def test_update_connection_with_proxy_id(self, mock_constants, mock_patch, mock_log, mock_ua):
        ...
        update_connection("cid", "cname", "gname", config, "pkg", "dkey", None, proxy_agent_id="proxy-123")
    ...
    @patch("fivetran_connector_sdk.connector_helper.rq.patch")
    @patch("fivetran_connector_sdk.connector_helper.constants")
    def test_update_connection_clears_proxy_when_none(self, mock_constants, mock_patch, mock_log, mock_ua):
        from fivetran_connector_sdk.connector_helper import update_connection
        mock_constants.PRODUCTION_BASE_URL = "http://test"
        mock_patch.return_value = MagicMock()

        config = {"schema": "s", "secrets_list": []}
        update_connection("cid", "cname", "gname", config, "pkg", "dkey", None, proxy_agent_id=None)
    ...
    @patch("fivetran_connector_sdk.connector_helper.rq.patch")
    @patch("fivetran_connector_sdk.connector_helper.constants")
    def test_update_connection_switches_from_proxy_to_directly(self, mock_constants, mock_patch, mock_log, mock_ua):
    ...
    def test_update_connection_preserves_proxy_host_config_key_when_proxy_active(
        ...
        config = {"schema": "s", "secrets_list": [{"key": "host", "value": "h1:5432"}],
...
class TestCreateConnectionWithProxy(unittest.TestCase):
    ...
    def test_create_connection_with_proxy_id(self, mock_constants, mock_post, mock_log, mock_ua):
        from fivetran_connector_sdk.connector_helper import create_connection, FIVETRAN_NAMING_VALUE, UNDERSCORE_NAMING
        mock_constants.PRODUCTION_BASE_URL = "http://test"
    ...
    @patch("fivetran_connector_sdk.connector_helper.rq.post")
    @patch("fivetran_connector_sdk.connector_helper.constants")
    def test_create_connection_without_proxy_id(self, mock_constants, mock_post, mock_log, mock_ua):
        from fivetran_connector_sdk.connector_helper import create_connection
        mock_constants.PRODUCTION_BASE_URL = "http://test"
        mock_post.return_value = MagicMock()
