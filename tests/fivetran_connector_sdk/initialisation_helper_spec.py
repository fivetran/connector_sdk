...
class TestInitialisationHelper(TestCase):
    ...
    @patch("sys.exit")
    def test_init_blank_project_runs_setup(
    ...
    def test_init_blank_project_non_interactive_runs_setup_and_skips_ai(
        ...
        mock_log.assert_any_call(
            "skipping AI agent setup; --non-interactive is set", log_icon=Logging.LogIcon.STEP
        )
        mock_exit.assert_called_once_with(0)
    ...
    def test_init_blank_project_yes_flag_runs_setup_and_skips_ai(
        ...
        mock_setup_ai_agent.assert_not_called()
        mock_log.assert_any_call(
            "skipping AI agent setup; --yes is set", log_icon=Logging.LogIcon.STEP
        )
        mock_exit.assert_called_once_with(0)
    ...
    def test_init_existing_project_force_overwrites_and_skips_ai(
        ...
        mock_setup_ai_agent.assert_not_called()
        mock_log.assert_any_call(
            "skipping AI agent setup; --force is set", log_icon=Logging.LogIcon.STEP
        )
        mock_exit.assert_called_once_with(0)
    ...
    def test_init_failure_during_setup(self, mock_exit, mock_log, mock_setup_connector):
        ...
        mock_log.assert_called_with(
            f"failed to initialize project error: {error_message}",
    ...
    def test_setup_connector_creates_directory(self, mock_makedirs, mock_log, mock_download):
        ...
        mock_log.assert_called_once_with(
            f"new project created at: {self.test_project_dir}", log_icon=Logging.LogIcon.SUCCESS
        )
    ...
    def test_agent_guidance_constants_are_derived_from_agent_plugin_config(self):
        ...
        self.assertIn(TOOLS_GITHUB_REPO_URL, AGENT_PLUGINS["gemini"]["install_commands"][0])
    ...
    def test_detect_installed_agents_returns_only_installed(self):
        ...
        def which_side_effect(cmd):
            return (
                "/usr/local/bin/claude" if cmd == AGENT_PLUGINS["claude"]["cli_command"] else None
            )
    ...
    def test_install_agent_plugin_runs_all_commands_for_claude(self, mock_log, mock_run):
        ...
        self.assertEqual(
            mock_run.call_args_list[0][0][0], AGENT_PLUGINS["claude"]["install_commands"][0]
        )
        self.assertEqual(
            mock_run.call_args_list[1][0][0], AGENT_PLUGINS["claude"]["install_commands"][1]
        )
    ...
    def test_install_agent_plugin_runs_single_command_for_gemini(self, mock_log, mock_run):
        ...
        self.assertEqual(
            mock_run.call_args_list[0][0][0], AGENT_PLUGINS["gemini"]["install_commands"][0]
        )
        self.assertTrue(result)
    ...
    def test_install_agent_plugin_handles_os_error(self, mock_log, mock_run):
        ...
        self.assertTrue(len(warning_calls) > 0)
        guidance_calls = [
            c for c in mock_log.call_args_list if "install the plugin later" in str(c)
        ]
        self.assertTrue(len(guidance_calls) > 0)
        self.assertTrue(
            any(AI_TOOLS_INSTALLATION_DOCS_URL in str(c) for c in mock_log.call_args_list)
        )

    @patch("fivetran_connector_sdk.initialisation_helper.install_agent_plugin")
    @patch(
        "fivetran_connector_sdk.initialisation_helper.detect_installed_agents",
        return_value={
    ...
    def test_setup_ai_agent_installs_first_agent_when_user_picks_1(
        self, mock_input, mock_detect, mock_install
    ):
    ...
    @patch("fivetran_connector_sdk.initialisation_helper.install_agent_plugin")
    @patch(
        "fivetran_connector_sdk.initialisation_helper.is_agent_plugin_install_supported",
        return_value=True,
    )
    @patch(
        "fivetran_connector_sdk.initialisation_helper.detect_installed_agents",
    ...
    @patch("builtins.input", return_value="2")
    def test_setup_ai_agent_skips_when_user_picks_skip_option(
        self, mock_input, mock_detect, mock_log, mock_install
    ):
    ...
    def test_setup_ai_agent_skips_on_invalid_input(
        self, mock_input, mock_detect, mock_log, mock_install
    ):
    ...
    def test_setup_ai_agent_skips_on_out_of_range_number(
        self, mock_input, mock_detect, mock_log, mock_install
    ):
    ...
    @patch("fivetran_connector_sdk.initialisation_helper.install_agent_plugin")
    @patch("fivetran_connector_sdk.initialisation_helper.print_library_log")
    @patch("fivetran_connector_sdk.initialisation_helper.detect_installed_agents", return_value={})
    def test_setup_ai_agent_logs_message_and_skips_when_no_agents_detected(
        self, mock_detect, mock_log, mock_install
    ):
    ...
    @patch("fivetran_connector_sdk.initialisation_helper.print_library_log")
    @patch("fivetran_connector_sdk.initialisation_helper.detect_installed_agents", return_value={})
    def test_setup_ai_agent_logs_constant_derived_guidance(
        self, mock_detect, mock_log, mock_install
    ):
        ...
        with patch(
            "fivetran_connector_sdk.initialisation_helper.SUPPORTED_AGENT_DISPLAY_NAMES",
            "Test Agent",
        ):
            with patch(
                "fivetran_connector_sdk.initialisation_helper.AI_TOOLS_INSTALLATION_DOCS_URL",
    ...
    def test_validate_example_directory_no_connector_file(self, mock_log):
        ...
        files_to_download = [{"local_path": "config.json"}, {"local_path": "helpers.py"}]
    ...
    def test_validate_example_directory_nested_connector_file(self):
        ...
        files_to_download = [{"local_path": "src/connector.py"}, {"local_path": "config.json"}]
    ...
    @patch("fivetran_connector_sdk.initialisation_helper.print_library_log")
    @patch("requests.get")
    def test_download_git_directory_success(
        self, mock_get, mock_log, mock_validate, mock_download_files
    ):
    ...
    def test_download_git_directory_api_error(self, mock_get, mock_log):
        ...
        self.assertTrue(
            any("failed to download files" in str(call) for call in mock_log.call_args_list)
        )
    ...
    def test_download_git_directory_no_files_found(self, mock_get, mock_log):
        ...
        mock_response.raise_for_status = MagicMock()
        mock_response.json.return_value = {
            "tree": [{"type": "blob", "path": "other/file.py", "size": 512}]
        }
        mock_get.return_value = mock_response
        ...
        self.assertTrue(
            any("no connector found matching" in str(call) for call in mock_log.call_args_list)
        )

    @patch("fivetran_connector_sdk.initialisation_helper.download_file_from_github")
    @patch("fivetran_connector_sdk.initialisation_helper.print_library_log")
    @patch("requests.get")
    def test_download_git_directory_excludes_readme_for_template(
        self, mock_get, mock_log, mock_download_files
    ):
    ...
    @patch("fivetran_connector_sdk.initialisation_helper.print_library_log")
    @patch("requests.get")
    def test_exact_match_does_not_include_prefix_collision(
        self, mock_get, mock_log, mock_validate, mock_download
    ):
    ...
    def test_prefix_match_skips_intermediate_directories(self, mock_get, mock_log):
        ...
        mock_response.json.return_value = {
            "tree": [
                ...
                {
                    "type": "blob",
                    "path": "examples/quickstart/hello_world/connector.py",
                    "size": 512,
                },
            ]
        }
    ...
    @patch("fivetran_connector_sdk.initialisation_helper.print_library_log")
    @patch("requests.get")
    def test_nested_example_prefix_match_lists_connectors_across_grouping_directories(
        ...
        mock_response.json.return_value = {
            "tree": [
                ...
                {
                    "type": "blob",
                    "path": "examples/advanced/hello_world/connector.py",
                    "size": 512,
                },
    ...
    @patch("fivetran_connector_sdk.initialisation_helper.print_library_log")
    @patch("requests.get")
    def test_trailing_slash_stripped_before_match(
        self, mock_get, mock_log, mock_validate, mock_download
    ):
    ...
    def test_download_file_from_github_success(
        self, mock_get, mock_makedirs, mock_exists, mock_file_open, mock_log
    ):
    ...
    @patch("requests.get")
    def test_download_file_from_github_overwrite_existing(
        self, mock_get, mock_exists, mock_file_open, mock_log
    ):
    ...
    @patch("os.makedirs")
    @patch("requests.get")
    def test_download_file_from_github_create_nested_directories(
        self, mock_get, mock_makedirs, mock_exists, mock_log
    ):
    ...
    @patch("os.makedirs")
    @patch("requests.get")
    def test_download_file_from_github_handles_download_error(
        self, mock_get, mock_makedirs, mock_exists, mock_log
    ):
        ...
        files_to_download = [
            {"github_path": "examples/test/missing.py", "local_path": "missing.py", "size": 100}
        ]
    ...
    @patch("os.makedirs")
    @patch("requests.get")
    def test_download_file_from_github_handles_write_error(
        self, mock_get, mock_makedirs, mock_exists, mock_file_open, mock_log
    ):
        ...
        files_to_download = [
            {"github_path": "examples/test/file.py", "local_path": "file.py", "size": 100}
        ]
    ...
    @patch("os.makedirs")
    @patch("requests.get")
    def test_download_file_from_github_file_in_root(
        self, mock_get, mock_makedirs, mock_dirname, mock_exists, mock_file_open, mock_log
    ):
    ...
    @patch("os.makedirs")
    @patch("requests.get")
    def test_download_file_from_github_multiple_files(
        self, mock_get, mock_makedirs, mock_exists, mock_file_open, mock_log
    ):
    ...
    @patch("os.makedirs")
    @patch("requests.get")
    def test_download_file_from_github_constructs_correct_url(
        self, mock_get, mock_makedirs, mock_exists, mock_file_open, mock_log
    ):
    ...
    @patch("os.makedirs")
    @patch("requests.get")
    def test_download_file_from_github_request_timeout(
        self, mock_get, mock_makedirs, mock_exists, mock_log
    ):
    ...
    def test_init_creates_project_with_template_connector(self, mock_input):
        ...
        self.assertTrue(
            os.path.exists(self.test_project_path),
        ...
        self.assertTrue(
            os.path.exists(connector_file), f"connector.py should exist at {connector_file}"
        )
    ...
    def test_init_non_interactive_keeps_existing_files(self, mock_input):
        ...
        with open(dummy_file, "r") as f:
            ...
            self.assertEqual(
                content,
                original_content,
                "File should NOT be overwritten with DEFAULT_ANSWER mode",
            )
    ...
    @patch("builtins.input")
    def test_init_with_claude_runs_plugin_install_commands(
        self, mock_input, mock_detect, mock_run
    ):
    ...
    def test_init_preserves_directory_structure(self, mock_input):
        ...
        expected_files = ["connector.py", "configuration.json", "requirements.txt"]

        for file_name in expected_files:
            file_path = os.path.join(self.test_project_path, file_name)
            self.assertTrue(
                os.path.exists(file_path), f"Expected file {file_name} should exist in project"
            )
    ...
    def test_init_excludes_readme_from_template(self, mock_input):
        ...
        for readme_file in readme_variations:
            ...
            self.assertFalse(
                os.path.exists(readme_path), f"{readme_file} should be excluded from template"
            )
