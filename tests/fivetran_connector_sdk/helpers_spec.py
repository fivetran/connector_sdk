...
class TestHelper(TestCase):
    ...
    def test_find_connector_object_success(
        self, mock_log, mock_module_from_spec, mock_spec_from_file_location
    ):
    ...
    @patch("importlib.util.module_from_spec")
    @patch("fivetran_connector_sdk.helpers.print_library_log")
    def test_find_connector_object_wrong_name(
        self, mock_log, mock_module_from_spec, mock_spec_from_file_location
    ):
        FakeConnector = type("Connector", (), {"__module__": "fivetran_connector_sdk"})
        mock_module = MagicMock()
    ...
    @patch("importlib.util.module_from_spec")
    @patch("fivetran_connector_sdk.helpers.print_library_log")
    def test_find_connector_object_no_connector_found(
        self, mock_log, mock_module_from_spec, mock_spec_from_file_location
    ):
        mock_module = MagicMock()
    ...
    @patch("importlib.util.module_from_spec")
    @patch("fivetran_connector_sdk.helpers.print_library_log")
    def test_find_connector_object_type_error(
        self, mock_log, mock_module_from_spec, mock_spec_from_file_location
    ):
        mock_module_from_spec.return_value = MagicMock()
        mock_loader = MagicMock()
        mock_loader.exec_module.side_effect = TypeError(
            "Connector.__init__() got an unexpected keyword argument 'test'"
        )
        mock_spec = MagicMock()
        mock_spec.loader = mock_loader
        mock_spec_from_file_location.return_value = mock_spec
        ...
        with patch.object(sys, "path", new=[]):
            ...
            self.assertIn(
                "Connector.__init__() got an unexpected keyword argument 'test'", logged_message
            )
            self.assertIn(
                "reference: https://fivetran.com/docs/connectors/connector-sdk/technical-reference",
                logged_message,
            )
            self.assertEqual(mock_log.call_args[0][1], Logging.Level.SEVERE)
    ...
    def test_print_library_log_executed_via_cli(self, mock_print):
        ...
        print_library_log(
            "cli message", Logging.Level.WARNING, log_icon=Logging.LogIcon.STEP, indent=True
        )
