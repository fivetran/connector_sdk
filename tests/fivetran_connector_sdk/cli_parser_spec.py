...
class TestCliParser(unittest.TestCase):
    ...
    def test_debug_help_is_context_sensitive(self):
        ...
        for unexpected in (
            ...
            self.assertNotIn(unexpected, out, f"debug help unexpectedly contains {unexpected}")
    ...
    def test_init_help_is_context_sensitive(self):
        ...
        for unexpected in (
            ...
            self.assertNotIn(unexpected, out, f"init help unexpectedly contains {unexpected}")
    ...
    def test_package_help_is_context_sensitive(self):
        ...
        for unexpected in (
            ...
            self.assertNotIn(unexpected, out, f"package help unexpectedly contains {unexpected}")
    ...
    def test_reset_help_is_context_sensitive(self):
        ...
        for unexpected in (
            ...
            self.assertNotIn(unexpected, out, f"reset help unexpectedly contains {unexpected}")
    ...
    def test_configuration_help_is_context_sensitive(self):
        ...
        for unexpected in (
            ...
            self.assertNotIn(
                unexpected, out, f"configuration help unexpectedly contains {unexpected}"
            )
    ...
    def test_typo_command_routes_to_suggester(self):
        ...
        with patch("sys.argv", ["fivetran", "depoly"]):
            with patch(
                "fivetran_connector_sdk.cli_parser.suggest_correct_command", return_value=True
            ) as mock_suggest:
                with self.assertRaises(SystemExit) as cm:
    ...
    def test_invalid_command_exits(self):
        ...
        with patch("sys.argv", ["fivetran", "totally-bogus-token"]):
            with patch(
                "fivetran_connector_sdk.cli_parser.suggest_correct_command", return_value=False
            ):
    ...
    def test_uppercase_command_is_normalised(self):
        ...
        argv = ["fivetran", "DEPLOY", "--api-key", "K", "--destination", "G", "--connection", "C"]
    ...
    def test_debug_proxy_id_without_value_reports_unsupported(self):
        ...
        self.assertIn(
            "Proxy Agent routing is not supported with `fivetran debug`.", buf.getvalue()
        )
    ...
    def test_debug_proxy_host_config_key_reports_unsupported(self):
        ...
        self.assertIn(
            "`--proxy-host-config-key` is only supported with `fivetran deploy`.", buf.getvalue()
        )
